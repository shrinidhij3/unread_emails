                ...
                if password:
                    try:
                        ...
                        if isinstance(password, str):
                            # Check for ENC: prefix or Fernet token format
...
async def process_inbox(cred, pool):
    ...
    if not imap_host:
        error_msg = f"Missing IMAP host for {email_address}"
        logger.error(f"❌ {error_msg}")
        await log_error(pool, email_address, "config_error", error_msg)
        return
        
    # Log connection details for debugging
    logger.debug(f"Connection details for {email_address}:")
    logger.debug(f"  - IMAP Host: {imap_host}")
    logger.debug(f"  - IMAP Port: {imap_port}")
    logger.debug(f"  - Use SSL: {use_ssl}")
    
    # Test DNS resolution
    try:
        ip_address = socket.gethostbyname(imap_host)
        logger.debug(f"  - Resolved IP: {ip_address}")
    except Exception as e:
        error_msg = f"DNS resolution failed for {imap_host}: {str(e)}"
        logger.error(f"❌ {error_msg}")
        await log_error(pool, email_address, "dns_error", error_msg)
        return
    ...
    if await is_rate_limited(pool, email_address):
        logger.warning(f"   ⚠️  Rate limited for {email_address}, skipping this cycle")
    ...
    for attempt in range(1, MAX_RETRY_ATTEMPTS + 1):
        try:
            logger.info(f"   [{attempt}/{MAX_RETRY_ATTEMPTS}] Connecting to {imap_host}:{imap_port} (SSL: {use_ssl})...")
            start_time = time.time()
            
            # Test port connectivity first
            try:
                test_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                test_socket.settimeout(IMAP_TIMEOUT)
                test_socket.connect((imap_host, imap_port))
                test_socket.close()
                logger.debug("    ✅ Port test successful")
            except Exception as e:
                error_msg = f"Port test failed for {imap_host}:{imap_port}: {str(e)}"
                logger.error(f"    ❌ {error_msg}")
                if attempt < MAX_RETRY_ATTEMPTS:
                    ...
                    continue
                await log_error(pool, email_address, "connection_error", error_msg)
                return
            ...
            try:
                ...
                mail.noop()
                logger.info(f"    ✅ Connected to {imap_host}:{imap_port} in {time.time() - start_time:.2f}s (SSL: {use_ssl})")
                
                # Login to the IMAP server
                try:
                    logger.info(f"    🔐 Attempting login as {email_address}...")
                    mail.login(email_address, cred['password'])
                    logger.info("    ✅ Login successful")
                    break  # Successfully connected and logged in
                ...
                if attempt < MAX_RETRY_ATTEMPTS:
                    ...
                    logger.error(f"    ❌ Max retries reached for {email_address}")
                    await log_error(pool, email_address, "connection_error", f"Failed after {MAX_RETRY_ATTEMPTS} attempts: {error_msg}")
                    return
    ...
    # If we get here and mail is None, all connection attempts failed
    if mail is None:
        error_msg = f"Failed to connect to {imap_host}:{imap_port} after {MAX_RETRY_ATTEMPTS} attempts"
        logger.error(f"❌ {error_msg}")
        await log_error(pool, email_address, "connection_error", error_msg)
        return
    else:
        logger.error(f"   ❌ Max retry attempts ({MAX_RETRY_ATTEMPTS}) exceeded for {email_address}")
        return
        ...
        if message_count > 0:
            ...
            if status == 'OK':
                ...
                for num in unread_messages:
                    try:
                        await process_single_email(mail, num, email_address, 'INBOX', pool)
                        processed_count += 1
                    except Exception as e:
                        logger.error(f"   ❌ Error processing message {num}: {str(e)}")
                        error_count += 1
                        await log_error(pool, email_address, 'message_processing_error', 
                                     f"Error processing message {num}: {str(e)}")
            
            logger.info(f"✅ Processed {processed_count} messages with {error_count} errors")
        else:
            logger.info("   No messages to process in INBOX")
        
    except Exception as e:
        logger.error(f"❌ Error processing mailbox: {str(e)}")
        logger.error(traceback.format_exc())
        await log_error(pool, email_address, 'mailbox_processing_error', 
                       f"Error processing mailbox: {str(e)}")
    finally:
        try:
            mail.logout()
            logger.info("Logged out from IMAP server")
        except Exception as e:
            logger.warning(f"   ⚠️  Error during logout: {str(e)}")
